...
pub struct Launch {
    ...
    pub total_owed: u64,
    /// Накопленный за всё время собранный налог (до сплита). Не в спеке
    /// дословно — это ончейн-прокси для `VOLUME_FLOOR`/`OG_VOLUME_FLOOR`
    /// (§3): оборот = собранный_налог / tax_bps, монотонен и не требует
    /// доверенного офчейн-входа для самого факта пересечения порога (в
    /// отличие от update_leader, который явно доверенный, см. §3.3 плана).
    pub lifetime_tax_collected: u64,
